...
function gatherConstraints(board) {
...
function subsetElimination(board) {
    let changed = false;
    ...
    for (const A of cons) {
        for (const B of cons) {
            ...
            if (A.cells.length >= B.cells.length)
                continue; // proper subset must be smaller
            // A ⊆ B ?
            let subset = true;
...
function solveOnce(board) {
    ...
    if (hidden.length > 0 && minesRemaining === 0) {
        hidden.forEach(c => { c.revealed = true; });
        changed = true;
    }
    else if (hidden.length > 0 && minesRemaining === hidden.length) {
        hidden.forEach(c => { c.flagged = true; });
        changed = true;
    }
    return changed;
}
...
function isFullySolvable(initialBoard, firstClick) {
    ...
    while (solveOnce(board)) {
        const unsound = board.flat().some(c => (c.flagged && !c.isMine) || // flagged a safe cell
    ...
    return board.every(row => row.every(c => c.isMine || c.revealed));
}
