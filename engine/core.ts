...
export function revealCell(board: Board, r: number, c: number): void {
...
// toggle flag
export function toggleFlag(board: Board, r: number, c: number): void {
  const cell = board[r][c]
  if (!cell.revealed) cell.flagged = !cell.flagged
}

// chord (auto‐clear) around a revealed number
export function chord(board: Board, r: number, c: number): void {
  const cell = board[r][c]
  ...
  neigh.forEach(n => {
    ...
    if (!nc.revealed && !nc.flagged) revealCell(board, n.r, n.c)
  })
}
